/**
 * Test: set_user_watch_project tool via mocked conversation
 *
 * Verifies that:
 * 1. The agent correctly calls set_user_watch_project when the LLM responds
...
describe("CodeAgent: set_user_watch_project via mock conversation", () => {
    let agent: CodeAgent
    let tmpDir: string
    let setDir: string
    ...
    it("should set working directory via tool call and update agent state", async () => {
        ...
        // Should have tool events for set_user_watch_project
        const toolStarts = events.filter((e) => e.type === "tool.start")
        expect(toolStarts.length).toBeGreaterThan(0)
        expect(toolStarts[0].toolName).toBe("set_user_watch_project")
